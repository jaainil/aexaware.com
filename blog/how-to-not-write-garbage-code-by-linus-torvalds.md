...
url: "https://aexaware.com/blog/how-to-not-write-garbage-code-by-linus-torvalds"
type: static
generatedAt: "2026-08-27T19:51:12.657Z"
---

# How to Not Write ‘Garbage Code’ (by Linus Torvalds)

If you ship software, you’ve seen “garbage code.” Linus Torvalds recently called out an example during the Linux merge window, and the lesson is timeless: don’t...
Jainil PrajapatiNovember 11, 2025![How to Not Write ‘Garbage Code’ (by Linus Torvalds)](/_astro/cover.Ezvq2_8U_Z19ax9x.webp)
...
![](/_astro/image-0.DkLT90lC_PKVog.webp)
...
![](/_astro/image-2.BOKvx_bF_XRvnE.webp)

 1. **Make intent local.** Keep meaning at the call site. Inline trivial operations; avoid generic helpers that hide semantics.
 1. **Model the data first.** Let structures drive behavior; simplify code by simplifying relationships.
 1. **Minimize context-switches.** Fewer levels of indirection → faster comprehension for humans and AI tools alike.
 1. **Delay abstractions.** Prove duplication hurts in practice before extracting helpers or frameworks.
 1. **Prefer clarity over cleverness.** Write the obvious operation explicitly when correctness or ordering matters.
 1. **Remove special cases.** Reshape data or control flow to collapse branches; aim for uniform operations.
 1. **Review for readability.** In code review, ask: “Can a new teammate explain this in one pass?” If not, simplify.
...
- [How to Not Write ‘Garbage Code’ (by Linus Torvalds): What He Meant](#how-to-not-write-garbage-code-by-linus-torvalds-what-he-meant)
- [Reduce cognitive load, not just lines of code](#reduce-cognitive-load-not-just-lines-of-code)
- [Prefer explicit code when semantics matter](#prefer-explicit-code-when-semantics-matter)
- [“Good taste” removes special cases](#good-taste-removes-special-cases)
- [Aexaware’s 7-point checklist to avoid “garbage code”](#aexawares-7-point-checklist-to-avoid-garbage-code)
- [Real-world example: from helper maze to readable service](#real-world-example-from-helper-maze-to-readable-service)
- [Guidelines for teams using AI coding tools](#guidelines-for-teams-using-ai-coding-tools)
- [Conclusion: How to Not Write ‘Garbage Code’ (by Linus Torvalds)](#conclusion-how-to-not-write-garbage-code-by-linus-torvalds)
- [Let’s build it right — with Aexaware](#lets-build-it-right--with-aexaware)
